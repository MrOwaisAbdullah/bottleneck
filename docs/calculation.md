...
The calculator applies a minimum value of 0 to ensure that bottleneck percentages are never negative, which could occur in edge cases where components are extremely mismatched.

### Resolution Adjustment Factors

Performance varies significantly with display resolution:
...
1. **RAM Amount**
   ...
   - For content creation tasks (video editing, 3D rendering), 32GB or more is recommended

2. **Storage Type**
   - NVMe SSD: Optimal performance with fastest read/write speeds
...
3. **Usage Purpose**
   ...
   - Video Editing: CPU-heavy workload with significant RAM requirements
   - 3D Rendering: CPU-heavy workload with GPU acceleration benefits
...
4. **Monitor Refresh Rate**
   - Affects target FPS for competitive gaming
   - Higher refresh rates (144Hz, 240Hz) require higher FPS for optimal experience

5. **Graphics Settings**
   - Low/High settings affect CPU/GPU workload distribution
   - Ray tracing settings significantly impact GPU performance
   - DLSS/FSR can improve performance on compatible hardware
...
These metrics provide users with a comprehensive view of their system's gaming performance, including both average performance and consistency metrics.
...
With Phase 2 implementation, the calculator now provides additional analysis:

1. **System Configuration Analysis**
   - RAM adequacy assessment
   - Storage performance evaluation
   - Usage match compatibility scoring

2. **Gaming Experience Prediction**
   - eSports performance estimates
   - AAA game performance predictions
...
- Status indicators (✓, △, ✗) show adequacy for the selected purpose
...
- SATA SSD: Good performance but slower than NVMe
- HDD: Significant impact on loading times and system responsiveness
- Status indicators show storage adequacy for the selected purpose
...
4. **Modular Architecture**
   - Each analysis module is implemented as a separate function
   - Results are dynamically populated into UI components
...
The modular architecture allows for easy extension with additional analysis features, and the responsive design ensures a consistent experience across all devices. By combining benchmark data with real-world usage patterns, the calculator provides actionable insights that help users get the most from their hardware investments.